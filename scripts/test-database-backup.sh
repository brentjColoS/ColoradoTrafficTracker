...
mkdir -p "$TEST_ROOT/bin" "$TEST_ROOT/backups" "$TEST_ROOT/receipts"
...
for _ in 1 2 3; do
  DATABASE_BACKUP_DIR="$TEST_ROOT/backups" \
  DATABASE_BACKUP_RETENTION_COUNT=2 \
...
backup_count="$(find "$TEST_ROOT/backups" -type f -name 'traffic-*.dump' | wc -l | tr -d ' ')"
manifest_count="$(find "$TEST_ROOT/backups" -type f -name 'traffic-*.dump.sha256' | wc -l | tr -d ' ')"
...
[[ ! -e "$TEST_ROOT/backups/traffic-20260801T033000Z.dump" ]] || { printf 'oldest dump was not removed\n' >&2; exit 1; }
