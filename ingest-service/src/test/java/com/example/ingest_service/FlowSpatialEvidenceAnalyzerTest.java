...
class FlowSpatialEvidenceAnalyzerTest {
    ...
    void summarizesUniqueDirectionalEvidenceWithoutNamingTravelDirections() {
        ...
        assertThat(evidence.decodedPathCount()).isEqualTo(3);
        assertThat(evidence.corridorPathCount()).isEqualTo(3);
        assertThat(evidence.uniquePathCount()).isEqualTo(2);
        assertThat(evidence.duplicatePathCount()).isEqualTo(1);
        assertThat(evidence.oneSidePathCount()).isEqualTo(2);
        assertThat(evidence.closurePathCount()).isEqualTo(1);
        assertThat(evidence.routeOrderForwardPathCount()).isEqualTo(1);
        assertThat(evidence.routeOrderReversePathCount()).isEqualTo(1);
        assertThat(evidence.ambiguousOrientationPathCount()).isZero();
    ...
    @Test
    void measuresOnlyMatchingPathsFromMultiPathFeatures() {
        DecodedTrafficFeature multiPath = new DecodedTrafficFeature(
            "Traffic flow",
            List.of(
                List.of(point(39.7020, -104.9998), point(39.7120, -104.9998)),
                List.of(point(39.7020, -104.9500), point(39.7320, -104.9500))
            ),
            Map.of(
                "road_type", "Motorway",
                "traffic_level", 42,
        ...
        assertThat(evidence.decodedPathCount()).isEqualTo(2);
        assertThat(evidence.corridorPathCount()).isEqualTo(1);
        assertThat(evidence.uniquePathCount()).isEqualTo(1);
        assertThat(evidence.pathLengthMiles().maximum()).isLessThan(1.0);
        assertThat(evidence.maxRouteDistanceMeters().maximum()).isLessThan(500.0);
    }
    ...
    void rejectsUnsupportedRoadsAndFeaturesOutsideTheCorridorBuffer() {
        ...
        assertThat(evidence.status()).isEqualTo("NO_MATCHING_PATHS");
        assertThat(evidence.decodedFeatureCount()).isEqualTo(2);
        assertThat(evidence.decodedPathCount()).isEqualTo(2);
        assertThat(evidence.corridorPathCount()).isZero();
        assertThat(evidence.uniquePathCount()).isZero();
        assertThat(evidence.pathLengthMiles().median()).isNull();
    }
    ...
    void reportsMissingRouteWithoutMakingSpatialClaims() {
        ...
        assertThat(evidence.decodedFeatureCount()).isEqualTo(1);
        assertThat(evidence.decodedPathCount()).isEqualTo(1);
        assertThat(evidence.uniquePathCount()).isZero();
